...
В корневой папке проекта создайте файл с расширением `.env`, в котором задайте значения переменным окружения по примеру из файла `.env.example`. Для запуска проекта необходимо запустить из консоли `index.php`, находясь в директории проекта, следующим образом:
